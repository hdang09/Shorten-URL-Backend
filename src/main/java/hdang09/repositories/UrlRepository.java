...
public interface UrlRepository extends CrudRepository<URL, Integer> {
    ...
    @Query("SELECT u FROM URL u WHERE u.originLink = :originLink AND u.account.id = :accountId")
    URL checkDuplicate(@Param("originLink") String originLink, @Param("accountId") int accountId);

    @Query("SELECT NEW hdang09.entities.Report(u.account.id, SUM(u.clicks), COUNT(u.shortenLink)) "
            + "FROM URL u WHERE u.account.id = :accountId")
    Report getReportByAccountId(@Param("accountId") int accountId);

    @Query("SELECT u FROM URL u WHERE u.account.id = :accountId")
    List<URL> getLinksByAccountId(@Param("accountId") int accountId);

    @Query("SELECT NEW hdang09.entities.Report(u.account.id, SUM(u.clicks), COUNT(u.shortenLink)) " +
            "FROM URL u WHERE u.account.id = :accountId  AND MONTH(u.createdAt) = :month AND YEAR(u.createdAt) = :year")
    ...
    @Query("SELECT u FROM URL u " +
            "WHERE u.account.id = :accountId AND MONTH(u.createdAt) = :month AND YEAR(u.createdAt) = :year")
